...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _viscous = 55.0f;//拉断距离
        _radius = 13.0f;//圆圈半径
        _bodyColor = [UIColor whiteColor];//圆圈内颜色
        _skinColor = [UIColor colorWithWhite:0.8f alpha:0.9f];
        _skinColor = [UIColor whiteColor];//圆圈颜色
        _missWhenApart = YES;
    }
    return self;
}
...
{
    UIBezierPath *path = [[UIBezierPath alloc] init];// lowoo autorelease
