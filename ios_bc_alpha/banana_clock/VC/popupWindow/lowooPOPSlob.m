...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        NSString *str = [BASE International:@"POPSlob"];
        ...
        _viewHead.view = [[UIView alloc] initWithFrame:CGRectMake(0, 170, 0, 0)];
...
- (void)buttonCloseTouchUpinside:(UIButton_custom *)sender{
    if ([_delegate respondsToSelector:@selector(buttonSlobTouchUpinsideWithentity:)]) {
        [_delegate buttonSlobTouchUpinsideWithentity:self];
    }
    
    [[NSNotificationCenter defaultCenter]postNotificationName:@"buttonSlobTouchUpinside" object:nil userInfo:nil];
